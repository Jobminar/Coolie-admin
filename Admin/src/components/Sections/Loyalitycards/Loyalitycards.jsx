...
const Loyalitycards = () => {
   ...
   const handlesubmit = async (e) => {
      ...
      try {
         ...
         const response = await fetch('http://13.126.118.3:3000/v1.0/admin/loyalty', {
            method: 'POST',
            body: formData,
         });
         if (response.ok) {
   ...
   useEffect(() => {
      axios.get('http://13.126.118.3:3000/v1.0/admin/loyalty')
   ...
   //  DElete
    const handleDelete = async (id) => {
      ...
      try {
        const response = await axios.delete(`http://13.126.118.3:3000/v1.0/admin/loyalty/${id}`);
      //   http://localhost:3000/v1.0/admin/loyalty/id
        if (response.status==200) {
          alert('Deleted successfully');
          setGetData(prevData => prevData.filter(loyality => loyality._id !== id));
        }
      } catch (err) {
