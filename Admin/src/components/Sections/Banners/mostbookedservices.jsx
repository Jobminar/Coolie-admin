...
const Mostbookedservices = () => {
  const navigate = useNavigate(2)
    ...
    const handleSelectChange = (event) => {
        ...
        const selected = fetchService.find(service => service._id === serviceId) || null;
    ...
    useEffect(() => {
        if (selectedService) {
            setFormData({
                ...
                price: selectedService.serviceVariants && selectedService.serviceVariants.length > 0 ? selectedService.serviceVariants[0].price : '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('http://13.126.118.3:3000/v1.0/admin/most-booked', {
                method: 'POST',
                body: data
            });
    ...
    useEffect(() => {
        const fetchMostBooked = async () => {
            try {
                ...
                console.error('Error while fetching the most booked services:', err.message);
            }
        };
        fetchMostBooked();
    }, []);
    ...
    const handleDelete = async (id) => {
      ...
      try {
        const response = await fetch(`http://13.126.118.3:3000/v1.0/admin/most-booked/${id}`, {
          method: "DELETE",
        });
  
        if (response.ok) {
          alert("Deleted successfully");
          setMostBookedData((prevData) => prevData.filter((banner) => banner._id !== id));
        } else {
